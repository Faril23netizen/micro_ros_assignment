...
extern volatile bool led_worker1_state;
extern volatile bool led_worker2_state;

	while (true) { // Loop forever
        if (xLedPad == 2) led_worker1_state = true;
        if (xLedPad == 3) led_worker2_state = true;
		gpio_put(xLedPad, 1);
		vTaskDelay(DELAY);
        if (xLedPad == 2) led_worker1_state = false;
        if (xLedPad == 3) led_worker2_state = false;
		gpio_put(xLedPad, 0);
